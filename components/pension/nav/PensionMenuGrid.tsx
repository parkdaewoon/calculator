  ...
  return (
    <section className="rounded-3xl border border-neutral-100 bg-white p-4 shadow-[0_10px_25px_rgba(0,0,0,0.05)]">
      <div className="grid grid-cols-2 gap-3">
        {MENU.map(({ key, title, desc, href, Icon }) => (
          <button
            key={key}
            type="button"
            onClick={() => router.push(href)}
            className="group w-full select-none touch-manipulation rounded-3xl border border-neutral-200 bg-white p-4 text-left transition hover:bg-neutral-50 active:scale-[0.98]"
          >
