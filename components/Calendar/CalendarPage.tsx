...
function buildReminderTitle(ev: CalendarEvent) {
  return ev?.title?.trim() || "일정 알림";
}
...
function buildReminderBody(ev: CalendarEvent, minutes: number) {
  ...
  if (minutes <= 0) return `${when} 일정 시간입니다.`;
  return `${when} 일정 ${minutes}분 전입니다.`;
}
...
async function showReminderNotification(ev: CalendarEvent, minutes: number) {
  ...
  const title = buildReminderTitle(ev);
