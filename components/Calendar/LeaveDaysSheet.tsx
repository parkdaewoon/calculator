...
export default function LeaveDaysSheet({ open, onClose }: LeaveDaysSheetProps) {
  ...
  // 열릴 때 localStorage 값 로드해서 input에 반영
  useEffect(() => {
    if (!open) return;
  ...
  const hasLogicError =
    parsedUsed != null && parsedTotal != null && parsedTotal > 0 && parsedUsed > parsedTotal;
  ...
  return (
    <Sheet open={open} onClose={onClose} title="연가일수 입력">
      <div className="space-y-3">
        <div className="rounded-2xl border border-neutral-100 bg-neutral-50 p-4">
          ...
          {/* 사용한 연가 */}
          <div className="mt-3">
            <div className="text-[11px] font-semibold text-neutral-500">사용한 연가일수</div>
            <div className="mt-2 flex items-center gap-2">
              ...
              />
              <div className="shrink-0 text-sm font-semibold text-neutral-700">일</div>
            </div>
            {parsedUsed == null && (
              <div className="mt-2 text-[11px] text-red-500">사용 일수는 숫자로 입력해줘.</div>
            )}
          </div>
        ...
        <button
          onClick={onClose}
          className="mt-1 w-full rounded-2xl bg-neutral-900/10 px-4 py-3 text-sm font-semibold text-neutral-800"
          type="button"
        >
          닫기
        </button>
